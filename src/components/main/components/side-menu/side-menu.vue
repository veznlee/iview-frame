<template>
  <div class="side-menu-wrapper">
    ...
    <div class="menu-collapsed" v-show="collapsed" :list="menuList">
      <template v-for="item in menuList">
        <collapsed-menu v-if="item.children && item.children.length > 1 || showChildren(item)" @on-click="handleSelect" hide-title :root-icon-size="rootIconSize" :icon-size="iconSize" :theme="theme" :parent-item="item" :key="`drop-menu-${item.name}`"></collapsed-menu>
...
export default {
  ...
  methods: {
    ...
    updateOpenName (name) {
      if (name === this.$appConfig.homeName) this.openedNames = []
